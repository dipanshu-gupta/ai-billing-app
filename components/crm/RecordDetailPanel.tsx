...
function ApprovalBanner({ recordId, recordName, recordType, onDecision }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Find pending request for this record
      ...
      const isUserApprover = stepData.approver_user_id === currentUser.id;

      // Also check via email match in enterprise_users
      let isApproverByEmail = false;
      if (!isUserApprover && currentUser.email) {
        const { data: eu } = await supabase.from('enterprise_users').select('id').eq('id', stepData.approver_user_id).single();
        isApproverByEmail = eu?.id === currentUser.id;
      }

      setIsApprover(isUserApprover || isApproverByEmail);
    };
    load();
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          ...
          <div className="text-sm text-gray-600 mb-1">
            Submitted by <span className="font-semibold">{request.submitted_by}</span>
          ...
          {isApprover ? (
            <div className="mt-4 space-y-3">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  disabled={deciding}
                  className="flex-1 bg-green-500 hover:bg-green-600 text-white py-3 rounded-2xl font-bold text-sm transition-all disabled:opacity-50 shadow-lg"
                >
                ...
                <button
                  ...
                  disabled={deciding}
                  className="flex-1 bg-red-500 hover:bg-red-600 text-white py-3 rounded-2xl font-bold text-sm transition-all disabled:opacity-50 shadow-lg"
                >
            ...
            <div className="mt-3 bg-white rounded-2xl border border-blue-200 px-4 py-3">
              <p className="text-sm text-blue-700">Waiting for approver to review this record. You will be notified when a decision is made.</p>
            </div>
          )}
        </div>
      </div>
...
function Customer360({ customer }) {
  ...
  const fmt = n => new Intl.NumberFormat('en-IN',{style:'currency',currency:'INR',maximumFractionDigits:0}).format(n);
  const secs = {
    contacts:      { icon:'📇', label:'Contacts',      data:contacts.filter(m),      cols:[{h:'Name',v:r=>r.name},{h:'Email',v:r=>r.email||'–'},{h:'Phone',v:r=>r.phone||'–'},{h:'Status',v:r=><Pill status={r.status}/>}] },
    leads:         { icon:'🎯', label:'Leads',          data:leads.filter(m),         cols:[{h:'Name',v:r=>r.name},{h:'Source',v:r=>r.source||'–'},{h:'Amount',v:r=>fmt(r.amount)},{h:'Status',v:r=><Pill status={r.status}/>}] },
    opportunities: { icon:'💼', label:'Opportunities',  data:opportunities.filter(m), cols:[{h:'Name',v:r=>r.name},{h:'Stage',v:r=>r.stage},{h:'Amount',v:r=>fmt(r.amount)},{h:'Status',v:r=><Pill status={r.status}/>}] },
    orders:        { icon:'🛒', label:'Orders',         data:orders.filter(m),        cols:[{h:'Name',v:r=>r.name},{h:'Amount',v:r=>fmt(r.amount)},{h:'Delivery',v:r=>r.deliveryDate||'–'},{h:'Status',v:r=><Pill status={r.status}/>}] },
    invoices:      { icon:'🧾', label:'Invoices',       data:invoices.filter(m),      cols:[{h:'Name',v:r=>r.name},{h:'Amount',v:r=>fmt(r.amount)},{h:'Due Date',v:r=>r.dueDate||'–'},{h:'Status',v:r=><Pill status={r.status}/>}] },
    activities:    { icon:'📅', label:'Activities',     data:activities.filter(m),    cols:[{h:'Name',v:r=>r.name},{h:'Type',v:r=>r.activityType||'–'},{h:'Date',v:r=>r.activityDate||'–'},{h:'Status',v:r=><Pill status={r.status}/>}] },
  };
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[110] overflow-y-auto">
      <div className="bg-white rounded-[28px] shadow-2xl w-[98vw] my-4 mx-auto overflow-hidden flex flex-col" style={{minHeight:'95vh'}}>
        ...
        <div className="flex-1 overflow-y-auto p-8 bg-gradient-to-br from-white to-blue-50 space-y-6">
          ...
          {/* ── APPROVAL BANNER — shown at the very top of details ── */}
          {(page!=='customers'||tab==='details') && ['leads','opportunities','orders','invoices','customers','contacts','activities'].includes(page) && (
            <ApprovalBanner
              recordId={record.id}
              recordName={record.name}
              recordType={page}
              onDecision={() => setEdited(p => ({...p, status: '—'}))}
            />
          )}
            ...
            <>
              ...
              <div className="bg-white rounded-[24px] border border-blue-100 shadow overflow-hidden">
                ...
                <div className="p-6 grid grid-cols-2 md:grid-cols-3 gap-4">
                  {[
                    ['Created By',record.created_by],['Created At',record.created_at?formatDateTime(record.created_at):'-'],
                    ['Updated By',record.updated_by],['Updated At',record.updated_at?formatDateTime(record.updated_at):'-'],
