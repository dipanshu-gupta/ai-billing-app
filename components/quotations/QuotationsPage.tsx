...
const buildQuoteHTML = (quote, items, template, products) => {
  ...
  const renderSection = (sec) => {
    ...
    switch(sec.type) {
      ...
      case 'quote_info': return `<div style="background:${s.bgColor};color:${s.textColor};padding:25px 50px;border-bottom:2px solid #E2E8F0;"><div style="display:grid;grid-template-columns:repeat(4,1fr);gap:20px;">${[['Quote Number',quote.quote_number||'-'],['Date',quote.created_at?formatDate(quote.created_at):''],['Valid Until',quote.validity_date||'-'],['Version',`v${quote.version||1}`],['Payment Terms',quote.payment_terms||''],['Currency',quote.currency||'INR']].filter(([,v])=>v).map(([l,v])=>`<div><div style="font-size:11px;text-transform:uppercase;opacity:0.6;margin-bottom:4px;">${l}</div><div style="font-weight:600;">${v}</div></div>`).join('')}</div></div>`;
